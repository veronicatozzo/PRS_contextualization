...
def get_controls(dems, encs, codes):
    ...
    print('Getting the last encounter date for controls..')
    controls = encs_controls.groupby(['ID'])['encounter_date'].agg(last_recorded_encounter_date='max').reset_index()
    print(f'Number of controls prior to ensuring at least 2 encounters {controls.shape[0]}')
